  ...
  function compressAndStore(device) {
    ...
    sharp(decoded, {
      ...
      .toBuffer((err, buffer, info) => {
        const Key = `${fileName}-${
          isBIOTC ? columns.biotc.name + "-" + device : device
        }.jpeg`;
        if (!err) {
            ...
            (error, data) => {
              if (error) {
                ...
                if (
                  ...
                  Item[columns.srcSet.name][columns.srcSet.type][
                    device === config.HANDHELD
                      ? config.HANDHELD_MAX_WIDTH
                      : device === config.TABLET
                      ? config.TABLET_MAX_WIDTH
                  ...
                  Item[columns.original.name][columns.original.type] =
                    data.Location;
                }
                console.log(
    ...
    if (axis === config.WIDTH) {
      ...
      if (isPortrait) {
        ...
        if (device === config.HANDHELD) {
          ...
          return Math.round(
            Number(config.BITOC_TABLET_WIDTH) * (resolution[1] / resolution[0])
          );
        } else if (device === config.LAPTOP) {
          return Math.round(
            Number(config.BITOC_LAPTOP_WIDTH) * (resolution[1] / resolution[0])
          );
        } else {
          return resolution[1];
        }
    ...
    try {
      ...
      if (isBIOTC) {
        ...
        if (checkItemResp[columns.updateTime.name]) {
          await updateExistingBiotcImage(
